...
def update_api_key_in_env(api_key: str):
    ...
    print(f"✅ API Key berhasil diupdate di {env_path}")
    return True
...
def update_api_key_in_config(api_key: str):
    ...
    print(f"✅ API Key berhasil diupdate di {config_path} (fallback)")
    return True
...
def main():
    ...
    try:
        ...
        print(f"⚠️  Warning: Gagal update .env: {e}")
    
    # Update di config.py sebagai fallback
    try:
        update_api_key_in_config(api_key)
    except Exception as e:
        print(f"❌ Error: Gagal update config.py: {e}")
        sys.exit(1)
    
    print("\n" + "=" * 70)
    print("✅ API Key berhasil diupdate!")
    print("=" * 70)
    print("\n📋 Langkah selanjutnya:")
